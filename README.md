...
This project demonstrates the following key concepts from the course (minimum 3 required):

| Key Concept | Status | Where to find it |
|---|---|---|
| Security features | ✅ | [`SECURITY.md`](./SECURITY.md) — full threat model. In code: `modify_data()` (human-in-the-loop guardrail), file size validation and CSV parse validation in `app.py` |
| Agent skills | ✅ | [`SKILL.md`](./.agent/skills/datascout-analysis/SKILL.md) — persona/behavior defined outside Python, loaded at runtime as `system_instruction` in `app.py` |
| Deployability | ✅ | Live app: [datascout-app.streamlit.app](https://datascout-app.streamlit.app/) — deployed on Streamlit Community Cloud, demoed in the video |

See the [\[demo video\]](https://www.youtube.com/watch?v=PBXFUvIAiTc) for Deployability shown live, and the sections below for full detail on each concept.
...
![DataScout proactive profiling on dataset upload](./screenshots/datascout-upload-greeting.png)
*The agent automatically profiles the uploaded CSV and proactively suggests business questions — no prompting required.*

![Interactive chart generated by DataScout](./screenshots/datascout-chart.png)
*An interactive Plotly histogram generated on request, with the agent contextualizing the result in its reply.*
...
![Human-in-the-loop confirmation guardrail](./screenshots/datascout-hitl-guardrail.png)
*When the user asks to "remove outliers," the agent does not mutate the data — it pauses and requests explicit human confirmation before `drop_outliers` runs.*
